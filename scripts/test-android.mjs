...
try {
  ...
  await page.locator('#close-settings').click(); await page.locator('#draft').fill('clear when hidden');
  await device.shell('input keyevent KEYCODE_BACK'); await tapHost('Host message');
  await expect(page.locator('#draft')).toHaveValue('');
  assert.deepEqual(errors, []);
  console.log('Android emulator E2E passed: visible private draft, ciphertext-only host insertion, real clipboard auto-decryption, full reader, Android Keystore persistence, key deletion, and hide-to-lock.');
} finally {
