...
5. On a disposable emulator, build the separate test host with `mobile/android/gradlew -p mobile/android :releaseHost:assembleDebug`. Set `ANDROID_SERIAL=emulator-...` and run `node scripts/test-release-android.mjs artifacts/releases/v<VERSION>/GachLagan-v<VERSION>-android.apk`. This installs the exact signed APK and exercises the host/keyboard clipboard flow through Android accessibility, without enabling WebView debugging. The host is a separate test APK and is never published.
